...
ExecuteProgram(
  ...
  VLOG(2) << "[clin-gpu-runner] execution done.";
...
GpuRunner::Run(GpuRunInputs run_inputs) {
  ...
  VLOG(2)
      << "[clin-gpu-runner] waiting for data transfer done (size of args to "
         "wait = "
      << transferred_args_to_wait.size() << ").";

#define CLIN_USE_ASYNC

#ifndef CLIN_USE_ASYNC
  run_inputs.host_ctx->Await(transferred_args_to_wait);
  VLOG(2) << "[clin-gpu-runner] data transfer done.";

  return ExecuteProgram(run_inputs, transferred_args, compilation_result,
